...
class Directory():
	...
	def __init__(self,path,nsteps,x0):
		...
		self.data = [self.get_data(file) for file in os.lisdtir(self.path)]
		self.row_data = self.get_rows() #list of lists of row data 
		self.col_data = self.get_cols() #list of lists of column data 
	...
	def get_rows(self):
		return [[filedata[640*i:640*(i+1)] for i in range(480)] for filedata in self.data]

	def get_cols(self):
		return [[filedata[480*i:480*(i+1)] for i in range(640)] for filedata in self.data]
